...
from shortCardiacBackend.transformPointsAndVectors import (
    find_smallest_distance_to_ref,
    clean_points,
    get_line,
    rotate_points,
    length,
    get_vector,
    calc_center_of_2_points,
)
...
def smooth_coord_resizing(coords, resize_polygon_factor, smoothing=False, mode="int"):
    ...
    if coords.shape[0] != 1 and smoothing:
        ...
        number_of_points = (
            np.pi * (3 * (d1 + d2) / 2 - np.sqrt(d1 * d2))
        ) / resize_polygon_factor
...
def coords_resizing(
    coords, resize_polygon_factor, smooth_resizing: bool = True, mode="int"
):
    """
    ...
    for key in coords.keys():
        coords[key] = smooth_coord_resizing(
            np.array(coords[key]), resize_polygon_factor, smooth_resizing, mode
        )
...
def find_ref_points(coords, config):
    ...
    coords["sacardialRefPoint"] = np.array(
        [calc_center_of_2_points(np.array(l_ref[0]), np.array(r_ref[0]))]
    )
    coords["sacardialInferiorRefPoint"] = np.array(
        [calc_center_of_2_points(np.array(l_ref[1]), np.array(r_ref[1]))]
    )
...
def correct_segmentations(coords, config):
    ...
    p2_l = find_smallest_distance_to_ref(
        left_ventricle, coords["sacardialInferiorRefPoint"]
    )
    ...
    p2_r = find_smallest_distance_to_ref(
        right_ventricel, coords["sacardialInferiorRefPoint"]
    )
    ...
    coords["sarvendocardialContourDorsalClosed"] = np.array(
        ...
        + list(
            get_line(
                ...
                / (0.5 * config.resize_polygon_factor),
                coords["sacardialRefPoint"][0] / (0.5 * config.resize_polygon_factor),
                numpy=True,
            )
            * 0.5
            * config.resize_polygon_factor
        )
    )
...
def calc_line_for_EI(roi, angle):
    ...
    for i in range(min(xs) + range_, max(xs) - range_ + 1):
        dists = [
            point_dict[str(k)][2] if point_dict.get(str(k)) is not None else None
            for k in range(i - range_, i + range_)
        ]
        ...
        if len(dists) > 0:
            ...
            if dist > max_dist:
                line = (
                    point_dict[str(i)][:2]
                    if point_dict.get(str(i)) is not None
                    else line
                )
...
def smooth_3D_mask(mask, scaling=2):
    ...
    return sitk.GetArrayFromImage(
        sitk.BinaryDilate(sitk.BinaryMorphologicalClosing(sitk.BinaryErode(sitk_mask)))
    )
...
def calc_mask_of_polygon(dcm_file, coord, name, scaling_factor):
    ...
    img = Image.new(
        "L", (dcm_shape[1] * scaling_factor, dcm_shape[0] * scaling_factor), 0
    )
    img_draw = ImageDraw.Draw(img)
    c = tuple(
        [(c1 * scaling_factor, c2 * scaling_factor) for c1, c2 in list(coord.get(name))]
    )
